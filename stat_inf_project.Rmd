...
# select variables of interest and group the data
gss %>% select(polviews, owngun) %>% na.omit() %>% filter(owngun != "Refused") %>% group_by(polviews, owngun) %>% summarise(counts = n()) %>% print(n=40)

# can use table to tally results into a contigency table
tmp <- gss %>% select(polviews, owngun) %>% na.omit() 
